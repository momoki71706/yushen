...
const CARD_MESSAGE_INSTRUCTION = `【记账卡片小语】请以你一贯的人设，根据下面小晴今天的记账记录，写一句简短的话（不超过20字），可以是关心她吃了什么好吃的、或者单纯调侃一下她今天的消费——要参考她记录里具体的类别或备注，不要写得笼统。语气自然像日常随口一说，不要写成分析、总结、推理的口吻（比如不要说"看你今天...说明/看来..."这类分析腔），不要提到"记账"这类词以外的程序化说法。只输出这句话本身，不要输出任何思考过程、前缀或解释。`;
...
// reflects what she's logged today. Deliberately today-only — no budget
// or other-month data — so it can't end up commenting on something she
// logged days ago as if it just happened.
